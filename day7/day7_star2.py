class OperatorTree:
    possible_tracker = []
    answer = 0
    def __init__(self, target, start_value, remaining_values, operator, input_index, previous_val):
        print("-------------------")
        self.target = target
        self.children = []
...
for index, i in enumerate(input_data):
    ...
    test_tree = OperatorTree(target=goal_value, start_value=start_val, remaining_values=values, operator=None, input_index=index, previous_value=start_val)
    print("debug")
